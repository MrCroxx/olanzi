...
- the visible effects of indicator enums on this firmware, timing units, and persistence after power-off; the field layout is now cross-checked against source code below
...
[CONFIRMED] Source cross-check: the [VibeKey Lite field table](https://github.com/arumwu/vibekey-lite/blob/43fb9017790838c454fc2159f3608d576c5c6e3a/docs/protocol.md#L270-L306) and [OpenVibeKey frame builders](https://github.com/palaemonboy/OpenVibeKey/blob/c9970faa126cb2ee355571ecd79901e397f49574/native/VibeKit/Sources/VibeKitHID/VibeKitDevice.swift#L92-L126) agree on GET `01 0b 88 01` and SET `01 0b 88 04`. Plaintext offset 4 is the field mask, 5 the light index, 6 the global mode, and 7 the all-on brightness. Four records starting at `8 + 5*i` contain `[type, workTime, breatheLevel, breatheBrightness, alwaysOnBrightness]`, with masks `0x04`, `0x08`, `0x10`, `0x20`, and `0x40`; global mode and brightness use `0x01` and `0x02`. This source layout supersedes the earlier speculative three-group interpretation.

[CONFIRMED] The implementation follows Studio’s brightness range below and the [OpenVibeKey mode/type enums](https://github.com/palaemonboy/OpenVibeKey/blob/c9970faa126cb2ee355571ecd79901e397f49574/native/VibeKit/Sources/VibeKitApp/ContentView.swift#L435-L478): global mode 0/1/2 (off/all on/work), all-on and per-light always-on/breathing brightness 0–20 (integer levels), and four lights in key 1/2/3/knob order with off or always-on types; only key 1 offers breathing. Opening the lighting tab reads automatically, and returning reads again without a draft; reads wait for connection or busy operations, while existing drafts are preserved. It reads before editing, preserves all other parameters in all four records, and compares another GET after writing. There is no background lighting polling or query loop after read failure. Apply failures preserve the draft and use the current readback as the next explicit retry baseline, without automatic resending. [INFERRED] Physical light order and enum meanings follow the reference implementation and remain unverified on this hardware; sustained breathing, timing units, and persistence are not promised. See [09 · Native App](09-native-macos.md) for usage.

[CONFIRMED] Static evidence from Studio: the local `/Applications/Ulanzi Studio.app/Contents/Resources/Ulanzi/UlanziDeck/version.txt` contains `version=3.3.9`. In its `Contents/MacOS/UlanziDeck`, `SettingDialog::SettingDialog(QString, QWidget*)` sets the lighting slider minimum to 0, maximum to 20, and step to 1 at `0x1003ac404` through `0x1003ac43c`. `SettingDialog::onLightBrightnessChanged()` passes the raw slider value to the work-mode knob (index 3) always-on brightness and key (index 0/1/2) breathing brightness at `0x1003b3974` through `0x1003b39bc`, and to all-on brightness at `0x1003b3a74` through `0x1003b3a80`, without scaling. In `Contents/Frameworks/kwdm.dylib`, `+[MessageHelper setDeviceIndicatorLightWorkModeBreatheBrightnessLevel:level:]` and `+[MessageHelper setDeviceIndicatorLightWorkModeAlwaysOnBrightnessLevel:level:]` write offsets `11 + 5*i` and `12 + 5*i` with masks `0x20` and `0x40`, respectively. This establishes Studio's operating range and work-mode dimming path, not every level's visible effect or linear intensity; 0–20 must not be labeled as percentages. The native app edits the brightness corresponding to each light's current type, preserving the other type's brightness and timing parameters.

[CONFIRMED] A local read-only query returned `81 0b 88 11 00 00 02 07 02 0a 02 07 02 02 0a 02 07 02 02 0a 02 07 02 01 0a 02 07 02`, confirming GET access `0x11`, raw mode 2, raw brightness 7, and raw light types 2/2/2/1. Brightness 7 is a level within the 0–20 range used by Studio, not a percentage; its actual light intensity has not been measured. Readings outside the known range must still be preserved. Reading type 2 does not prove sustained breathing on that light.

[CONFIRMED] On 2026-09-24, a local mode write and restore were read back: changing mode 2 to 0 with mask `0x01` produced GET offset 6 equal to 0, with offsets 7…27 unchanged. Restoring mode 2 produced GET offsets 6…27 identical to the original values. This confirms mode-field read/write and this restoration, without establishing visible light changes; per-light type writes still require hardware verification; brightness tests follow below.

[CONFIRMED] Brightness writes on the same day: all-on brightness changed from 1 to 2 with mask `0x02`, and key 1 breathing brightness changed from 7 to 8 with mask `0x20`, both confirmed by GET. Attempts to change knob always-on brightness from 2 to 3, 1, 0, and 20 using mask `0x40`, focus index 3, and offset 27 all read back as 2, with other fields unchanged. This preserves the earlier readback mismatch but does not establish failed writes: subsequent full-payload `0x40`, full-payload `0x7c`, and Studio-style sparse-payload writes all received access `0x14` ACKs echoing 8, while GET still returned 2. The user explicitly observed the knob become dimmer then brighter when commanded from 0 to 20; the write worked, and treating unchanged GET as failure was incorrect. A restoration command for value 2 was then acknowledged; because GET is unreliable for this field, it cannot establish restoration of the earlier physical intensity. See the [lighting verification record](evidence/2026-09-24-knob-brightness-readback.log) for key frames.

[CONFIRMED] The fix waits for a `0x14` ACK matching each request's command, field mask, light index, and selected field values. Only knob always-on brightness excludes GET offset 27 from success verification and displays the most recent ACK-confirmed command value within the connection; raw GET data is preserved rather than fabricated. Without a confirmed value on first connection, the UI shows `—`; reconnecting or going offline clears the session value. Other fields still require GET comparison. The session value represents this app's most recently confirmed setting, cannot track changes from external tools, and does not establish power-off persistence or every level's physical effect.

### Next Steps (ordered by value)

1. ~~Reprogram key 1~~ ✅ **done**, see Section 6
2. **Verify indicator lights** — the native app now uses `setDeviceIndicatorLight*` (`01 0b 88 04`) for manual controls and readback comparison;
   physical effects, persistence, and AI-state integration still require verification
