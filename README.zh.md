...
- **后台运行。** 关闭窗口后隐藏 Dock 图标，继续使用已保存的动作，需要修改时从菜单栏重新打开。
...
目前支持 **AU05 的按键与旋钮动作及灯效控制**。
打开 **灯效标签页** 会自动读取当前配置。选择全局模式、0–20 档全亮亮度，或各灯工作模式与对应的 0–20 档亮度，再应用改动。
旋钮亮度 0 → 20 的实体变化已确认，其他灯效与断电后的持久化仍待验证。
设备该字段回读陈旧，显示值采用本连接最近确认的命令值；详见[原生 App 指南](docs/09-native-macos.zh.md)。
应用暂不支持固件更新、多媒体输出或其他 Ulanzi 设备。
